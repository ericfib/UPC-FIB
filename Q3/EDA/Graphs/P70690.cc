...
bool DFS_busca(const MC& map, int i, int j) {
  int n = map.size();
  int m = map[0].size();
        ...
        for(int i = 0; i < 4; i++) {
          ...
          if (pos_ok(new_i, new_j, m, n)) {
            if (map[new_i][new_j] != 'X') {
              if (map[new_i][new_j] == 't') return true;
              else S.push({new_i, new_j});
            }
          }
        }
      }
...
int main() {
  ...
  if(DFS_busca(mapa, orig_i-1, orig_j-1)) cout << "yes" << endl;
  else cout << "no" << endl;
