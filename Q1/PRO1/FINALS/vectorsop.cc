...
int search_element(const vector<string>& v, string x, int esq, int dre) {
    if (esq > dre) return -1;
    int pos = (esq+dre)/2;
    if (x > v[pos]) return search_element(v,x,pos+1,dre);
    if (x < v[pos]) return search_element(v,x,esq,pos-1);
    return pos;
}
...
void element_insertion(vector<string>& v, string x) {
       int n = v.size();
       int pos = search_element(v,x,0,n-1);
       if (pos == -1){
         v.push_back(x);
         for(int i = n; i >= 1; i--){
           if (v[i] < v[i-1]){
